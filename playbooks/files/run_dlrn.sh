...
#NOTE(jpena): this is a temporary hack to properly use kerberos for git cloning
kinit -kt ~/.keytab jpena@REDHAT.COM
#dlrn --config-file /usr/local/share/dlrn/${USER}/projects.ini --info-repo /home/rdoinfo/rdoinfo/ ${DLRN_ENV:-} "$@" 2>> $LOGFILE
dlrn --config-file /usr/local/share/dlrn/${USER}/projects.ini ${DLRN_ENV:-} "$@" 2>> $LOGFILE
RET=$?
